...
## 2026-08-11 - Accessible tooltips on disabled buttons issue fix
**Learning:** The previous learning "Accessible Tooltips on Disabled Buttons" used `peer-focus-visible` to keep the tooltip accessible without showing it on mouse click due to `group-focus-within`. However, replacing `group` with `peer` on a wrapper element breaks the hover state of inner elements (like `group-hover:scale-110`). If a tooltip is a sibling, the button needs `peer` and the tooltip needs `peer-hover`. If the tooltip is a child of the button, the button needs `group` and the tooltip needs `group-hover`.
**Action:** When migrating from `group` to `peer` to fix tooltips on disabled elements, ensure that you only apply `peer` when the tooltip is a sibling of the trigger element. Remove the `group` class from the wrapper to isolate the state. Do not replace `group` with `peer` if the tooltip is a child of the trigger element.
...
## 2026-08-21 - Accessible Keyboard Hints and Tooltip Visibility
**Learning:** Visual keyboard hints (like `<kbd>Enter</kbd>`) inside interactive elements can cause screen readers to redundantly read the hint alongside the main label if not explicitly hidden using `aria-hidden="true"`. Additionally, relying on `group-focus:opacity-100` instead of `group-focus-visible:opacity-100` causes these hints to incorrectly appear and linger when the element is clicked with a mouse. Finally, absolutely positioned tooltips (like in the RedCardGuide sidebars) can be clipped by overlapping layouts if they lack a sufficiently high `z-index` like `z-50`.
**Action:** Use `group-focus-visible` instead of `group-focus` for child tooltips or visual keyboard hints to ensure they only appear during keyboard navigation. Always hide these hints from screen readers with `aria-hidden="true"`, and ensure custom absolutely positioned tooltips have an appropriate `z-index` to prevent visual clipping.
