...
## 2026-07-11 - [Optimize Controlled Inputs in Heavy Parent Components]
**Learning:** Using controlled React state (e.g. `useState`) for an input (like a `<textarea>`) inside a parent component that also renders a large list (like `TiebreakerScreen` rendering the tiebreaker `checklist`) forces the entire parent and the list to re-render on every single keystroke. This O(N) penalty makes typing feel sluggish as the list grows.
**Action:** When an input's value does not need to immediately trigger UI changes elsewhere on every keystroke, switch to an uncontrolled input using `useRef`. Read the ref's current value only when necessary (e.g., on a submit button click).

## 2026-07-11 - [Optimize List Item Rendering with React.memo]
**Learning:** In a dynamic list where items can individually change status (e.g. checking/unchecking items in the Tiebreaker checklist), mapping inline JSX elements causes the entire list to re-render whenever the parent state updates, even if only one item changed.
**Action:** Extract list items into standalone components wrapped in `React.memo()`, and ensure the parent's update handlers passed to them are memoized with `useCallback`. This isolates re-renders to only the specific item that actually changed.
